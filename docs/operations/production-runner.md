...
`scripts/pipeline/preflight_video_factory.py` runs before GPU allocation and validates the source file,
...
```bash
python scripts/pipeline/run_production.py data/input/script.txt --serial
```

Stop after a stage:

```bash
python scripts/pipeline/run_production.py data/input/script.txt --through phase6-keyframes
```
