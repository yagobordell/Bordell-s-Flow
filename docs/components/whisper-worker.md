...
The no-prompt profile is deployed from the versioned `whisper-large-v3-turbo-v5` image tag. The
immutable digest is resolved and recorded by `Prepare` before validation.

Historical validation evidence and the deployment issues found along the way are preserved in Git
history; this document describes the active worker contract.
