...
def render_flux2_klein_prompt(prompt: str) -> str:
    """Render the canonical structured image prompt into concise narrative prose."""
...
def build_flux2_klein_job_request(
    *,
    task_name: str,
    prompt: str,
    model_id: str,
    ...
    if task_name not in _SUPPORTED_TASKS:
        raise ValueError(f"Unsupported FLUX.2 Klein provider task: {task_name}")
    if model_id != FLUX2_KLEIN_MODEL_ID:
        raise ValueError(f"FLUX.2 Klein provider requires model {FLUX2_KLEIN_MODEL_ID!r}")
    rendered = render_flux2_klein_prompt(prompt)
    ...
    def _generate_image_sync(
        ...
        *,
        ...
        if quality not in {"high", "auto"}:
            raise ValueError("FLUX.2 Klein fallback supports quality='high' or 'auto'")
        if output_format != "png":
            raise ValueError("FLUX.2 Klein fallback supports only PNG output")
        ...
        if not content:
            raise RuntimeError("FLUX.2 Klein fallback returned an empty PNG artifact")
        return GeneratedImage(
            ...
            metadata={
                ...
                "prompt_sha256": hashlib.sha256(
                    render_flux2_klein_prompt(prompt).encode()
                ).hexdigest(),
            },
        )
...
def _parse_size(size: str) -> tuple[int, int]:
    try:
        ...
        raise ValueError(f"Invalid FLUX.2 Klein image size: {size!r}") from exc
    if width % 16 or height % 16 or width < 256 or height < 256:
        raise ValueError("FLUX.2 Klein dimensions must be >=256 and divisible by 16")
