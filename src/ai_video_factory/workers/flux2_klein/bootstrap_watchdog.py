...
def watch_bootstrap(
    ...
    *,
    ...
    started = time.time()
    print(
        "FLUX2_KLEIN_BOOTSTRAP_WATCHDOG "
    ...
    while True:
        ...
        print(
            "FLUX2_KLEIN_BOOTSTRAP_PROGRESS "
            f"stage={stage} elapsed_seconds={elapsed:.1f} stage_seconds={stage_age:.1f}",
            flush=True,
        )
        if stage == _TERMINAL_STAGE:
            print("FLUX2_KLEIN_BOOTSTRAP_WATCHDOG_COMPLETE", flush=True)
            return

        reason: str | None = None
        if elapsed >= hard_timeout_seconds:
            reason = (
                "FLUX.2 Klein runtime bootstrap exceeded hard timeout of "
                f"{hard_timeout_seconds:g} seconds"
            )
        elif stage_age >= stage_timeout_seconds:
            reason = (
                f"FLUX.2 Klein runtime bootstrap stage {stage!r} made no stage progress for "
                f"{stage_timeout_seconds:g} seconds"
            )

        if reason is not None:
            print(f"FLUX2_KLEIN_BOOTSTRAP_TIMEOUT reason={reason}", flush=True)
            if reallocate_on_stall:
                requested = request_salad_reallocation(reason)
                print(
                    "SALAD_REALLOCATION_REQUEST "
                    "source=flux2_klein_runtime_bootstrap "
                    f"requested={str(requested).lower()} reason={reason}",
                    flush=True,
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Watch FLUX.2 Klein runtime bootstrap progress.")
