...
start_app() {
  uvicorn ai_video_factory.workers.qwen_image_21.runtime:app \
    --host 0.0.0.0 \
    --port 8080 \
    --no-access-log
}
...
if ! wait_for_health; then
  echo "Qwen-Image-2.1 worker HTTP health endpoint did not start" >&2
  exit 1
fi
...
if ! wait_for_ready; then
  echo "Qwen-Image-2.1 worker did not become ready after model bootstrap" >&2
  exit 1
fi

if [[ "${SALAD_QUEUE_ENABLED:-false}" != "true" ]]; then
  echo "Qwen-Image-2.1 worker ready; queue transport disabled"
  set +e
  wait "${app_pid}"
  status=$?
  set -e
...
echo "Qwen-Image-2.1 worker ready; starting Salad queue transport"
