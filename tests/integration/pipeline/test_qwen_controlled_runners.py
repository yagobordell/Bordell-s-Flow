from pathlib import Path

PHASE4 = Path("scripts/pipeline/run_phase4_assets_controlled.ps1")
PHASE6 = Path("scripts/pipeline/run_phase6_keyframes_controlled.ps1")
SHARED = Path("scripts/pipeline/_qwen_controlled.ps1")


def test_controlled_image_runners_use_qwen_optimized_prewarm() -> None:
    shared = SHARED.read_text(encoding="utf-8")
    assert '"../salad/start_salad_optimized_prewarm.ps1"' in shared
    assert 'Service = "qwen_image_21"' in shared
    assert '"arm_salad_scale_to_zero.ps1"' not in shared
    for path in (PHASE4, PHASE6):
        assert "_qwen_controlled.ps1" in path.read_text(encoding="utf-8")


def test_phase4_plan_flattens_json_array_before_counting_hits() -> None:
    shared = SHARED.read_text(encoding="utf-8")
    assert "ConvertFrom-Json |" in shared
    assert "ForEach-Object { $_ }" in shared
    assert '@($Plan | Where-Object { $_.status -eq "hit" }).Count' in shared
