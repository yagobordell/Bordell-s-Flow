...
function Import-EnvFile {
    ...
    foreach ($RawLine in Get-Content -LiteralPath $Resolved) {
        ...
        if ($Line -notmatch '^(?:export\s+)?(?<name>[A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?<value>.*)$') { continue }
        $Name = $Matches["name"]
...
function Get-Headers {
    return @{
        ...
        "User-Agent" = "ai-video-factory-queue-repair/1.0"
    }
}
...
function Try-Get-Group {
    try { return Invoke-RestMethod -Uri $GroupUrl -Headers $Headers -TimeoutSec 30 }
    catch {
...
function Get-Queue { return Invoke-RestMethod -Uri $QueueUrl -Headers $Headers -TimeoutSec 30 }

function Test-QueueAttachment {
    param([Parameter(Mandatory)][object]$Queue)
    return @(@($Queue.container_groups) | Where-Object { [string]$_.name -eq $GroupName }).Count -eq 1
}
...
function Test-GroupConfiguration {
    ...
    if ($null -eq $Autoscaler -or $null -eq $Autoscaler.Value) { return $false }
    if ($null -eq $Networking -or $null -eq $Networking.Value) { return $false }
    return (
...
function Wait-ForGroupDeleted {
    ...
    do {
        ...
        if ($null -eq (Try-Get-Group)) { return }
...
function Wait-ForGroupSettled {
    ...
    do {
        ...
        $Group = Try-Get-Group
        if ($null -ne $Group -and -not [bool]$Group.pending_change) { return $Group }
    } while ((Get-Date) -lt $Deadline)
...
function Get-WorkerEnvironment {
    ...
    foreach ($Property in $Definition.environment.PSObject.Properties) {
        ...
        if ([string]::IsNullOrWhiteSpace($Override)) { $Environment[$Property.Name] = [string]$Property.Value }
        else { $Environment[$Property.Name] = $Override.Trim() }
    }
    $Required = @($Document.stack.shared_required_environment | ForEach-Object { [string]$_ })
    $Required += @($Definition.required_environment | ForEach-Object { [string]$_ })
    $SecretNames = @("POSTGRES_DSN", "R2_ACCESS_KEY_ID", "R2_SECRET_ACCESS_KEY", "HF_TOKEN")
    foreach ($Name in @($Required | Select-Object -Unique)) {
        $Environment[$Name] = Get-Setting -Name $Name -Prompt $Name -Secret:($SecretNames -contains $Name)
    }
    return $Environment
}
...
function New-QueueConnection { return @{ path = $QueuePath; port = $ContainerPort; queue_name = $QueueName } }
function New-Networking { return @{ protocol = "http"; port = $ContainerPort; auth = $true } }

function New-CreateBody {
    param([Parameter(Mandatory)][object]$ExistingGroup)
    $GpuClasses = @($ExistingGroup.container.resources.gpu_classes | ForEach-Object { [string]$_ })
    if ($GpuClasses.Count -eq 0) { throw "Existing group '$GroupName' has no resolved GPU classes to preserve." }
    return @{
        name = $GroupName
...
if (-not (Test-Path -LiteralPath $ManifestPath -PathType Leaf)) { throw "Salad stack manifest not found: $ManifestPath" }
$Document = Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json
$ServiceProperty = $Document.services.PSObject.Properties[$Service]
if ($null -eq $ServiceProperty) { throw "Unknown Salad service '$Service'." }
...
$Group = Try-Get-Group
if ($null -eq $Group) { throw "Container group '$GroupName' is missing. Run Prepare first." }
$Queue = Get-Queue
...
if ([string]$Group.current_state.status -ne "stopped" -or [int]$Group.replicas -ne 0) {
    throw "Queue repair requires '$GroupName' to be stopped at zero replicas."
}
if ([int]$Queue.current_queue_length -ne 0) {
    throw "Queue '$QueueName' contains $($Queue.current_queue_length) job(s). Cancel them before repairing the container group."
}

$CreateBody = New-CreateBody -ExistingGroup $Group | ConvertTo-Json -Depth 20
Write-Warning "Recreating stopped container group '$GroupName' so Salad can attach Job Queue autoscaling."
Invoke-RestMethod -Method Delete -Uri $GroupUrl -Headers $Headers -TimeoutSec 60 | Out-Null
Wait-ForGroupDeleted
Invoke-RestMethod -Method Post -Uri "$Base/containers" -Headers $Headers -ContentType "application/json" -Body $CreateBody -TimeoutSec 60 | Out-Null
$Group = Wait-ForGroupSettled
if ($null -eq $Group.PSObject.Properties["queue_autoscaler"] -or $null -eq $Group.queue_autoscaler) {
    $PatchBody = @{ networking = New-Networking; queue_connection = New-QueueConnection; queue_autoscaler = New-QueueAutoscaler } | ConvertTo-Json -Depth 10
    Invoke-RestMethod -Method Patch -Uri $GroupUrl -Headers $Headers -ContentType "application/merge-patch+json" -Body $PatchBody -TimeoutSec 60 | Out-Null
    $Group = Wait-ForGroupSettled
}
$Queue = Get-Queue
if (-not (Test-GroupConfiguration -Group $Group)) { throw "Salad did not persist the complete Job Queue autoscaling configuration." }
if (-not (Test-QueueAttachment -Queue $Queue)) { throw "Salad did not attach '$GroupName' to Job Queue '$QueueName'." }
if ([int]$Group.replicas -ne 0) { throw "Queue repair unexpectedly started replicas for '$GroupName'." }
Write-Host "$Service Job Queue autoscaling repaired and verified; group remains at zero replicas." -ForegroundColor Green
