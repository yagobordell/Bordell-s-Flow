...
function Import-EnvFile {
    ...
    if (-not [IO.Path]::IsPathRooted($Resolved)) {
        $Resolved = Join-Path [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..")) $Resolved
    }
...
$ScaleToZeroStarter = Join-Path $PSScriptRoot "../salad/start_salad_scale_to_zero.ps1"
$OptimizedPrewarm = Join-Path $PSScriptRoot "../salad/start_salad_optimized_prewarm.ps1"
$QueueGuard = Join-Path $PSScriptRoot "../salad/check_salad_queue_ready.py"
...
$ManifestInspector = Join-Path $PSScriptRoot "../diagnostics/inspect_phase8_upscale_manifest.py"
$FailureInspector = Join-Path $PSScriptRoot "../diagnostics/inspect_inference_job_error.py"
$Runner = Join-Path $PSScriptRoot "run_phase8_upscale.py"
$ServicesPath = Join-Path [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..")) "deploy\salad\services.json"
