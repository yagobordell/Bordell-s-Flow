...
$Headers = @{
    ...
    "User-Agent" = "ai-video-factory-flux2-klein-restore/1.3"
}
...
Write-Host "FLUX.2 Klein restore: applying manifest queue autoscaler settings." -ForegroundColor Cyan
Invoke-RestMethod `
    -Method Patch `
    -Uri $GroupUrl `
    -Headers $Headers `
...
if ([int]$Group.replicas -ne 0) {
    Write-Host (
        "FLUX.2 Klein restore: normalizing desired replicas=$([int]$Group.replicas) to zero in a separate patch."
...
if ($Status -ne "stopped") {
    Write-Host "FLUX.2 Klein restore: stopping fallback worker group." -ForegroundColor Cyan
    Invoke-RestMethod `
        -Method Post `
        -Uri "$GroupUrl/stop" `
        -Headers $Headers `
...
do {
    ...
    if ($Status -eq "stopped" -and -not [bool]$Group.pending_change -and [int]$Group.replicas -eq 0) {
        Write-Host "FLUX.2 Klein fallback restored: stopped at zero replicas with manifest scale-to-zero." -ForegroundColor Green
        exit 0
    }
}
...
throw (
    "FLUX.2 Klein fallback did not stop cleanly before timeout; " +
