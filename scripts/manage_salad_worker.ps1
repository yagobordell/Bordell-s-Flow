...
function Assert-ServiceDefinition {
    ...
    if ($Image -notmatch '^[^\s]+:[^\s/:]+    if ($QueuePath -ne "/jobs") {
...
switch ($Action) {
    ...
    "Prepare" {
        ...
        elseif (
            ...
            [string]$ExistingGroup.container.image -match '@sha256:[0-9a-fA-F]{64}        $WorkerEnvironment = Get-WorkerEnvironment
    ...
    if (
        ...
        $PinnedImage -notmatch '^[^\s]+@sha256:[0-9a-fA-F]{64}    if ($QueuePath -ne "/jobs") {
        throw "Service '$Service' queue path must remain /jobs for the shared worker HTTP contract."
    }
    if ($ContainerPort -ne 8080) {
        throw "Service '$Service' container port must remain 8080 for the shared worker contract."
